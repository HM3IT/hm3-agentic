...
class ServerSettings:
    ...
    HTTP_WORKERS: int | None = field(
        default_factory=get_env("LITESTAR_HTTP_WORKERS", None)
    )
...
class ChatSettings:
    ...
    CHAT_API_BASE: str = field(default_factory=get_env("CHAT_API_BASE", ""))
    """base url of chat API"""
    API_BASE_URL: str = field(default_factory=get_env("API_BASE_URL", ""))
    """base url that the chat will try to connect to."""
    MODEL_API_KEY: str = field(default_factory=get_env("MODEL_API_KEY", ""))
    """API key for the model."""
    MODEL_BASE_URL: str = field(default_factory=get_env("MODEL_BASE_URL", ""))
    """base url for the model."""
    MODEL_NAME: str = field(default_factory=get_env("MODEL_NAME", "Claude-3.5-Sonnet"))
    """name of the model."""
    MODEL_FAMILY: str = field(
        default_factory=get_env("MODEL_FAMILY", "claude-3-5-sonnet")
    )
    """family of the model."""
    USER_AGENT_NAME: str = field(
        default_factory=get_env("USER_AGENT_NAME", "h3m-agent/1.0")
    )
    """User agent name for the model."""
    ...
    REDDIT_CLIENT_SECRET: str = field(
        default_factory=get_env("REDDIT_CLIENT_SECRET", "")
    )
